...
create_clock -name clk -period $CLK_50_PERIOD [get_ports {clk}]
...
# SoCkit edition does not use PLL, 10M comes direclty from ADC
create_clock -period 100.000 [get_ports adc_clk_in]
...
create_generated_clock -name linduino_sck \
        -source [get_pins {spi_if_inst|sclk|q}] \
        [get_ports {linduino_sck}]
...
set_clock_groups -asynchronous \
        -group [get_clocks {clk adc_clk_in}] \
        -group [get_clocks {adc_clk  dac_sclk linduino_sck}] \
...
set_input_delay -clock adc_clk -max [expr $CLK_SKEW_MAX + $ADC_TCO_MAX + $TRACE_DLY_MAX] \
        -clock_fall [get_ports {adc_data*}]
set_input_delay -clock adc_clk -min [expr $CLK_SKEW_MIN + $ADC_TCO_MIN + $TRACE_DLY_MIN] \
        -clock_fall [get_ports {adc_data*}]

# Constrain output pins going to DAC.
set_output_delay -clock linduino_sck -max [expr $SPI_SKEW_MAX + $DAC_TSU] [get_ports {linduino_mosi}]
set_output_delay -clock linduino_sck -min [expr $SPI_SKEW_MIN - $DAC_TH]  [get_ports {linduino_mosi}]
...
set_multicycle_path -from adc_clk_in -to linduino_sck -start -hold  1
